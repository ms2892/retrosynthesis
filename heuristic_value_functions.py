...
class GNNSplitNNEstimator(NoCacheNodeEvaluator):
    def __init__(self,scale,inventory,mode='max',**kwargs):
        super().__init__(**kwargs)
        self.scale=scale
        self.model = torch.load('GNN_split_Multi_Sample_NN_Model.pth')
    ...
    def get_nn_dist(self,smiles):
        ...
        if brics:
            ...
            for reps in range(REPEAT):
                ...
                action = np.random.choice(len(edges.detach().numpy()),p=edges.detach().numpy())
                bond = brics[action]
                ...
                if self.mode=='max':
                    nns.append(np.max(nn_dist))
                elif self.mode=='avg':
                    nns.append(np.mean(nn_dist))
            return max(nns)
        else:
...
class GNN_NNSplitSingleFunction(NoCacheNodeEvaluator):

    def __init__(self,scale,inventory,mode='max',**kwargs):
        super().__init__(**kwargs)
        self.scale=scale
        self.model = torch.load('GNN_split_NN_Model.pth')
        self.device = torch.device('cpu')
        self.model.to(self.device)
        self.model.eval()
        self._set_fingerprints([mol.smiles for mol in inventory.purchasable_mols()])
