...
if __name__ == "__main__":
    ...
    inventory = PaRoutesRandomInventory(state=0,n=args.paroutes_n)
    ...
    for scale in [3.0,10.0]:
        # NN SINGLE cost heuristic (different scale)
        value_fns.append(
            (
                f"GNN_NNSplitSingleMAXScore-{scale}",
                GNN_NNSplitSingleFunction(
                    scale=scale,
                    inventory=inventory,
                    mode='max'
                ),
    ...
    for scale in [3.0,10.0]:
        ...
        value_fns.append(
            (
                f"GNN_NNSplitSingleAVGSCore-{scale}",
                GNN_NNSplitSingleFunction(
                    scale=scale,
                    inventory=inventory,
                    mode='avg'
